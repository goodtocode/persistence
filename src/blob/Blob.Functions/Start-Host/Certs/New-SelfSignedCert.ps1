...
#      .\New-SelfSignedCert -Path .\dev.mysundial.com.pfx -Password MySundial1234 -Dns dev.mysundial.com

param (
	[Parameter(Mandatory=$true,ValueFromPipelineByPropertyName=$true)]
 	[string]$Path = $(throw '-Path is a required parameter.'), #.\Certs\MyCert.pfx
    [Parameter(Mandatory=$true,ValueFromPipelineByPropertyName=$true)]
    [string]$Password = $(throw '-Password is a required parameter.'), #MySundial1234
    [Parameter(Mandatory=$true,ValueFromPipelineByPropertyName=$true)]
	[string]$Dns = $(throw '-Dns is a required parameter.'), #dev.mysundial.com
